...
def fetch_remaining_images(last_media_id, post_id, current_image_count, save_dir=None):
    """Fetch remaining images metadata without downloading to disk"""
...
def extract_media(node, post_id, save_dir=None):
...
def post_already_exists(post_id, base_folder=None, name_folder=None):
...
def fetch_posts(limit=10, min_comments=0, batch_size=10, on_batch_complete=None):
    ...
    while len(all_posts) < limit:
        ...
        for node in story_nodes:
            ...
            is_existing = post_already_exists(post_id)
            if is_existing:
                print(f"  ℹ️  Post {post_id} already exists in DB -> will fetch to update comments")
            ...
            # Extract media in-memory
            post["media"] = extract_media(node, post_id)
